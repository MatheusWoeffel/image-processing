...
export function quantizeImage(
  image: Image,
  numberOfShadesDesired: number,
  numberOfChannels: number
): Image {
  image = convertToGrayScale(image, numberOfChannels);

  const shadesData = getShadesDataFromImage(image);
  const numberOfShadesFromImage =
    shadesData.mostLuminanceShade - shadesData.lessLuminanceShade + 1;
  if (numberOfShadesFromImage <= numberOfShadesDesired) return image;

  const shadesSpectrum = generateShadesSpectrum(
    shadesData,
    numberOfShadesDesired
  );
  ...
  return {
    width: image.width,
    height: image.height,
    data: Buffer.concat(imageInPixels),
  };
}
...
function generateShadesSpectrum(
  shadesData: ShadesData,
  numberOfShadesDesired: number
): ShadesSpectrum {
...
function getQuantizedShade(
  shadesSpectrum: ShadesBin[],
  pixelValue: number
): number {
